...
\begin{itemize}
  ...
  \item \textbf{v2.6.1 (2026/08/12)} --- Internal consistency fix with no change to typeset output. The rule for inserting the boundary ghost character after ruby and okurigana is now uniform: no ghost is emitted when a cooperating command (\verb|\送り| or \verb|\返り|) follows, since those position themselves against the parent character, and a ghost is emitted before any other following character including \verb|、| and \verb|。|. Previously \verb|\振り| emitted a ghost before a following \verb|\送り|/\verb|\返り| and omitted it before \verb|。|, neither of which matched \verb|\グ振り|. The change was verified by rebuilding every tracked PDF and comparing the rendered pages at 60, 100 and 150 dpi; all seven were identical. Also removed three edge-test cases that applied \verb|intrusion| to a return mark with no overhang and no \verb|\KanHyphen|, where the full width cancellation made the return mark collide with the following character.
\end{itemize}
